...
def write_lesson_sheet(spreadsheet, flow_name: str, meeting_dt: datetime, teacher_name: str, df: pd.DataFrame):
    ...
    ensure_header(worksheet, COLUMNS)
    apply_sheet_basics(spreadsheet, worksheet, len(COLUMNS))
    ...
    title_row = [lesson_title] + [""] * (len(COLUMNS) - 1)
    spacer_row = [""] * len(COLUMNS)
    rows_to_append = [spacer_row, title_row] + df[COLUMNS].values.tolist()

    required_rows = next_row + len(rows_to_append) + 10
    ensure_data_rows(worksheet, required_rows, len(COLUMNS))
    gspread_with_retry(worksheet.append_rows, rows_to_append, value_input_option="RAW")
    format_lesson_block(spreadsheet, worksheet, next_row + 1, len(df), len(COLUMNS))
    return sheet_name
...
def export_dashboard_site(spreadsheets: dict):
    ...
    for product, spreadsheet in spreadsheets.items():
        ...
        for lesson in lessons:
            ...
            bucket["teachers"].add(lesson.get("teacher", ""))
            bucket["student_count"] += int(lesson.get("student_count", 0))
            ...
            month_list.append(
                {
                    ...
                    "student_total": bucket["student_count"],
                }
            )
        ...
        dashboard["products"][product] = {
            ...
            "flow_count": len(sorted({lesson.get("flow", "") for lesson in lessons if lesson.get("flow")})),
            "teacher_count": len(sorted({lesson.get("teacher", "") for lesson in lessons if lesson.get("teacher")})),
            "months": month_list,
            "lessons": lessons,
